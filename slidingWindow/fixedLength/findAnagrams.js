...
var findAnagrams = function (s, p) {
  ...
  const K = p.length;
  const N = s.length;
  //total uniue characters required
...
/*
compat from above 
Runtime: 11 ms Beats 90.27%
Memory: 60.18 MB Beats 53.99%
*/

var findAnagrams = function (s, p) {
  //create map of char and its count
  const pCharCount = new Map();
  for (let c of p) {
    pCharCount.set(c, (pCharCount.get(c) || 0) + 1);
  }
  ...
  while (j < N) {
    const char = s[j];
    if (pCharCount.has(char)) {
      //char found in 'pat' then reduce the char count
      pCharCount.set(char, pCharCount.get(char) - 1);
      //all ocuurance of this char matched, then we need to match one less char
      if (pCharCount.get(char) == 0) count--;
    }
    if (j - i + 1 == K) {
      // we found match for all characters
      if (count == 0) result.push(i);
      const charOut = s[i];
      //remove the left char
      if (pCharCount.has(charOut)) {
        const charCountOut = pCharCount.get(charOut);
        //all occurence of this char was already matched and since we are removing it
        //we need increase the count for unique char required
        if (charCountOut == 0) count++;
        pCharCount.set(charOut, charCountOut + 1);
      }
      i++;
    }
    j++;

...
/* 
Approach III: Using two pointers
Time: O(N)
Space: O(1)

Runtime: 0 ms Beats 100.00% 
memory: 52.37 MB Beats 53.66%
*/
var checkValidString = function (s) {
  const N = s.length;
  let minCount = 0;
  ...
  for (let c of s) {
    if (c === "(") {
      ...
      //if "*" becomes ")" then decrease min count
      minCount--;
      //if "*" becomes "(" then increase max count
      maxCount++;
    }
    //we don't have enoughb parenthesis to match close parenthesis
    if (maxCount < 0) return false;
    //we can not have min count negative so set it to 0 if so
    minCount = Math.max(minCount, 0);
  }
  //return true if we found open count = 0 in range [minCount, maxCount]
  return minCount == 0;
};

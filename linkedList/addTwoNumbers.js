...
var addTwoNumbers = function(l1, l2) {
  ...
  for (let k = result.length - 1; k >=0; k--){
    ...
    // by doing num % 10 we get the digit to add
    result[k] = digitSum % 10;
    // with parseInt(digitSum / 10) we get carry over
  ...
  console.log(`leadingZeroCount: ${leadingZeroCount}`);
  let head = null;
  //O(M+N)
  for (let i = leadingZeroCount; i < result.length; i++){
     const node = new ListNode(result[i]);
     //add at front to get in sum digits in reverse order
     node.next = head;
     head = node;
  }
  console.log(head);

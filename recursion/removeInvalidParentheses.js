...
var removeInvalidParentheses = function (s) {
  ...
  const backtrack = (s, validString, stack) => {
    ...
    if (
      second == ")" &&
      secondStack.length &&
      secondStack[secondStack.length - 1] == "("
    ) {
      secondStack.pop();
...
var removeInvalidParentheses = function (s) {
  ...
  const dfs = (s, pos, str, rmL, rmR, open) => {
    console.log(
      `pos: ${pos}, str: ${str}, rmL: ${rmL}, rmR: ${rmR}, open: ${open}`
    );
...
var removeInvalidParentheses = function (s) {
  ...
  let resultSet = new Set();
  const dfs = (s, pos, str, rmL, rmR, open) => {
    //if we have already removed invalid left and right parentheses then we can't proceed further
    if (rmL < 0 || rmR < 0 || open < 0) {
      return;
    }
    //we reached end of string
    if (pos == s.length) {
      //if valid string (any string with count of left or right parentheses not 0 is invalid)
      if (rmL == 0 && rmR == 0 && open == 0) {
        resultSet.add(str);
      }
      return;
    ...
    if (s[pos] == "(") {
      //use '('
      dfs(s, pos + 1, str + "(", rmL, rmR, open + 1);
      //do not use '('
      dfs(s, pos + 1, str, rmL - 1, rmR, open);
    } else if (s[pos] == ")") {
      //use ')'
